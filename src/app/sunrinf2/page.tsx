    ...
    return (
        <main className="min-h-[calc(100vh-64px)] flex flex-col gap-6 mx-auto max-w-6xl">
            <div className="px-4 sm:px-6 py-8 space-between">
                ...
                <div>
                    <h2 className="text-xl font-bold mt-6 mb-4">방명록</h2>
                    {guestbooks.length === 0 ? (
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                            {guestbooks.map((guestbook: any) => (
                                <div
                                    key={guestbook.id}
                                    className="rounded-xl border border-white/10 bg-black/30 p-4 transition-colors hover:bg-black/40"
                                >
                                    <div className="flex items-start gap-3">
                                        <div className="h-9 w-9 shrink-0 rounded-full bg-gradient-to-br from-sky-500/40 to-violet-500/40 flex items-center justify-center text-white/80 text-sm font-semibold">
                                            <Image src={guestbook.authorPhoto || "/avatar.png"} className="rounded-full" alt="author" width={32} height={32} />
                                        </div>
